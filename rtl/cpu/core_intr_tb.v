...
module merc32_intr_tb();
    ...
    always @(posedge clk) begin
        if (cpu_inst.prog_step) begin
            case({prog_data[7:4], prog_data[3:0]})
                ...
                default: begin
                    $display("[%0d] : Unknown opcode! data=%h", prog_addr, prog_data);
                end
            endcase
        end
    end
    ...
    //--------------------------------------------------------------------------
    localparam INTR_PERIOD  = 600;
    localparam INTR_WIDTH   = 80;

    reg [31:0] intr_cnt = 0;
    reg [31:0] intr_fired = 0;
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (intr_fired >= 3) begin
                interrupt <= 1'b0;
            end else if (intr_cnt >= INTR_PERIOD) begin
                if (interrupt == 1'b0) begin
                    interrupt <= 1'b1;
                    intr_fired <= intr_fired + 1;
                    $display("--- [INTR] Rising-edge interrupt #%0d at time %0t ---", intr_fired + 1, $time);
                end

                if (intr_cnt >= INTR_PERIOD + INTR_WIDTH) begin
                    interrupt <= 1'b0;
                    intr_cnt  <= 0;
                end
            end
    ...
    //--------------------------------------------------------------------------
    // Interrupt handler monitor
    //--------------------------------------------------------------------------
    reg         in_handler = 0;
    reg [31:0]  intr_enter_cnt = 0;
    reg [31:0]  intr_exit_cnt = 0;
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (cpu_inst.prog_step) begin
                if (prog_addr >= 5 && prog_addr <= 13 && !in_handler) begin
                    in_handler <= 1;
                    intr_enter_cnt <= intr_enter_cnt + 1;
                    $display(">>> [ENTER ISR] addr=%0d", prog_addr);
                end else if (prog_addr < 5 && in_handler) begin
                    in_handler <= 0;
                    intr_exit_cnt <= intr_exit_cnt + 1;
                    $display("<<< [EXIT  ISR] addr=%0d", prog_addr);
                end
            end
        end
    end
    ...
    initial begin
        $dumpfile("merc32_intr_tb.vcd");
        $dumpvars(0, merc32_intr_tb);
    end
    ...
    task summary;
        begin
            ...
            $display("  ISR entered      : %0d", intr_enter_cnt);
            $display("  ISR exited       : %0d", intr_exit_cnt);
            if (intr_enter_cnt > 0 && intr_enter_cnt == intr_exit_cnt) begin
                $display("  Result           : PASS");
            end else if (intr_enter_cnt == 0) begin
                $display("  Result           : FAIL (no interrupt caught)");
            end else begin
                $display("  Result           : FAIL (enter/exit mismatch)");
            end
            $display("============================================");
        end
    endtask
